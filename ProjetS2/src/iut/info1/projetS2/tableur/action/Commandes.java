...
public class Commandes {
     ...
     */
    protected static int[] recupCase(String aRecuperer) {
    ...
    private static String adapteCalc(String aAdapter, int nbDecLig,
        ...
        String tmpChar; // stock temporairement un caractère
        Matcher testeur;
        ...
        for (int i = 0; i < lol.size(); i++) {
            if (lol.get(i).matches(REG_MODIF2)) { // lettre/chiffre
                ...
                //    System.out.println(j + " = " + testeur.group(j));
                //}
                tmpChar // recupère la colonne modifiée ou non en cas de '$'
                = colonneLocked(lol.get(i)) ? "$".concat(testeur.group(4))
                        : String.valueOf(
                                (char)(testeur.group(4).charAt(0) + nbDecCol));
                tmp // recupère la ligne modifiée ou non en cas de '$'
                = ligneLocked(lol.get(i)) ? "$".concat(testeur.group(7))
                        : String.valueOf(
                                Integer.parseInt(testeur.group(7)) + nbDecLig);
        ...
        if (aCopier.matches(REG_CASE) && ouCopier.matches(REG_CASE)) {
            ...
            for (int i = coordFinalInit[0]; i <= coordFinalFinal[0]; i++) {
                for (int j = coordFinalInit[1]; j <= coordFinalFinal[1]; j++) {
                    tmp = adapteCalc(entrees[coordInitInit[0]][coordInitInit[1]],
                            i - coordInitInit[0], j - coordInitInit[1]);
                    affichage(tmp);
                }
            }
            this.fenetre.getLabel().setText("Transfert effectué");
            ...
            if (copieOk) {
                ...
                System.out.println("x " + xPattern + " y " + yPattern);
                // copie
                for (int i = coordFinalInit[0]; i<=coordFinalFinal[0];i++) {
                    for (int j = coordFinalInit[1]; j<=coordFinalFinal[1];j++) {
                        tmp = adapteCalc(entrees[coordInitInit[0]+(i%xPattern)]
                                [coordInitInit[1] + (j % yPattern)],
                                i - coordInitInit[0] - i % xPattern, 
                                j - coordInitInit[1] - j % yPattern);
                        affichage(tmp);
                    }
                }
                this.fenetre.getLabel().setText("Transfert effectué");
                ...
                xPattern = 0;
                yPattern = 0;
                this.fenetre.getLabel().setText("Transfert impossible pour "
                        + "les plages entrées");
            }
        }
    }
    ...
    private void raz(String aEffacer) {
        ...
        if (siPlage.matches()) {
            ...
            //            }
            // récupère les cases composant la plage
            Scanner recup = new Scanner(aEffacer);
            recup.useDelimiter("\\0056{2}");
